...
export default function ItemOutwardScreen() {
  ...
  const fetchItemOutwards = async (p = page, s = search, isRefreshing = false) => {
    if (!isRefreshing) setLoading(true);
    setError('');
    try {
      const response = await api.get('/item-outward/datatable', {
      ...
      if (response.data.status) {
        ...
        setError('Failed to fetch item outwards');
      }
    } catch (err) {
      setError(err.response?.data?.message || 'Could not load item outwards');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  useEffect(() => {
    fetchItemOutwards(page, search);
    fetchItemsAutocomplete();
  }, [page]);

  const handleRefresh = () => {
    setRefreshing(true);
    fetchItemOutwards(page, search, true);
  };

  const handleSearchChange = (val) => {
    setSearch(val);
    setPage(1);
    fetchItemOutwards(1, val);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await api.post('/item-outward/store', { 
        date, 
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.headerRow}>
        <Text style={styles.headerTitle}>Item Outward</Text>
      ...
      <View style={styles.searchBar}>
        ...
        <TextInput 
          placeholder="Search item outward..."
      ...
      >
        {loading && !refreshing ? (
          <Text style={styles.infoText}>Loading item outwards...</Text>
        ) : error ? (
          <Text style={styles.errorText}>{error}</Text>
        ) : itemsList.length === 0 ? (
          <Text style={styles.infoText}>No item outwards found.</Text>
        ) : (
        ...
        <View style={styles.formModalOverlay}>
          <View style={styles.formModalContent}>
            <View style={styles.formModalHeader}>
              <Text style={styles.formModalTitle}>Add Item Outward</Text>
