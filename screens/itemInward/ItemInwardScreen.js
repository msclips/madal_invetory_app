...
export default function ItemInwardScreen() {
  ...
  const fetchItemInwards = async (p = page, s = search, isRefreshing = false) => {
    if (!isRefreshing) setLoading(true);
    setError('');
    try {
      const response = await api.get('/item-inward/datatable', {
      ...
      if (response.data.status) {
        ...
        setError('Failed to fetch item inwards');
      }
    } catch (err) {
      setError(err.response?.data?.message || 'Could not load item inwards');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  useEffect(() => {
    fetchItemInwards(page, search);
    fetchItemsAutocomplete();
  }, [page]);

  const handleRefresh = () => {
    setRefreshing(true);
    fetchItemInwards(page, search, true);
  };

  const handleSearchChange = (val) => {
    setSearch(val);
    setPage(1);
    fetchItemInwards(1, val);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await api.post('/item-inward/store', { 
        date, 
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.headerRow}>
        <Text style={styles.headerTitle}>Item Inward</Text>
      ...
      <View style={styles.searchBar}>
        ...
        <TextInput 
          placeholder="Search item inward..."
      ...
      >
        {loading && !refreshing ? (
          <Text style={styles.infoText}>Loading item inwards...</Text>
        ) : error ? (
          <Text style={styles.errorText}>{error}</Text>
        ) : itemsList.length === 0 ? (
          <Text style={styles.infoText}>No item inwards found.</Text>
        ) : (
        ...
        <View style={styles.formModalOverlay}>
          <View style={styles.formModalContent}>
            <View style={styles.formModalHeader}>
              <Text style={styles.formModalTitle}>Add Item Inward</Text>
